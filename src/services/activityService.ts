...
export const activityService = {
  ...
  async getActivities(filters?: ActivityListFilters & { page?: number; limit?: number; committee_ids?: number[]; overdue_only?: boolean }): Promise<{ activities: Activity[]; total: number }> {
    const params = new URLSearchParams();
    if (filters?.committee_id) params.append('committee_id', filters.committee_id.toString());
    if (filters?.committee_ids) {
      filters.committee_ids.forEach(id => params.append('committee_ids', id.toString()));
    }
  ...
  async updateActivity(id: number, activity: ActivityUpdate): Promise<Activity> {
  ...
  async completeActivity(id: number, completionNotes?: string): Promise<Activity> {
  ...
  async updateActivityStatus(id: number, status: ActivityStatus, notes?: string): Promise<Activity> {
  ...
  async updateActivityPriority(id: number, priority: ActivityPriority): Promise<Activity> {
  ...
  async getActivityStatistics(committeeId: number, dateFrom?: string, dateTo?: string): Promise<any> {
    ...
    const response = await api.get(`${BASE_URL}/statistics/${committeeId}?${params.toString()}`);
    return response.data;
  },
  ...
  async getActivityStatuses(): Promise<{ value: ActivityStatus; label: string }[]> {
    return [
  ...
  async getActivityPriorities(): Promise<{ value: ActivityPriority; label: string }[]> {
    return [
  ...
  async getPendingActivities(committeeId: number): Promise<Activity[]> {
    const response = await api.get(`${BASE_URL}?committee_id=${committeeId}&status=pending`);
    return response.data;
  },
  ...
  // Bulk operations
  async bulkUpdateStatus(activityIds: number[], status: ActivityStatus): Promise<Activity[]> {
  ...
  // Export activities
  async exportActivities(filters?: ActivityListFilters, format: 'pdf' | 'excel' = 'excel'): Promise<Blob> {
    const params = new URLSearchParams();
    if (filters?.committee_id) params.append('committee_id', filters.committee_id.toString());
    if (filters?.assigned_to) params.append('assigned_to', filters.assigned_to.toString());
    if (filters?.status) params.append('status', filters.status);
    ...
    const response = await api.get(`${BASE_URL}/export/${format}?${params.toString()}`, {
      responseType: 'blob',
    });
    return response.data;
  },
  ...
  async scheduleActivityReminder(activityId: number, reminderDate: string): Promise<void> {

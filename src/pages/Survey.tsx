...
// User Survey Answer Interface (removed due to no usage)
...
const Survey: React.FC = () => {
  ...
  const [viewingSurvey, setViewingSurvey] = useState<SurveyData | null>(null);
  ...
  const [openDetailedResponseDialog, setOpenDetailedResponseDialog] =
    useState(false);
  const [selectedEmployeeResponse, setSelectedEmployeeResponse] =
...
useEffect(() => {
    ...
    try {
      ...
      if (surveyToRespond.course_id) {
        try {
          ...
          if (
            courseProgress.pending_surveys &&
            courseProgress.pending_surveys.length === 0
          ) {
      ...
      try {
        ...
        if (error.response?.status === 400) {
          ...
          errorMessage =
            ...
            "No se puede eliminar la encuesta porque tiene respuestas asociadas";
  ...
  const handleDuplicateSurvey = async (survey: SurveyData) => {
    try {
      ...
      const duplicatedQuestions =
        fullSurveyData.questions?.map((question: SurveyQuestion) => {
          const { id, survey_id, created_at, updated_at, ...questionData } =
            question;
          return questionData;
        }) || [];
  ...
  const handleAssignSurvey = (survey: SurveyData) => {
  ...
  const getFilteredUsers = () => {
    return workers.filter((worker) => {
      ...
      const matchesArea =
        userAreaFilter === "" || (worker.department || "") === userAreaFilter;
      const matchesCargo =
        userCargoFilter === "" || (worker.position || "") === userCargoFilter;
  ...
  const handleAddQuestion = () => {
    ...
    const question: SurveyQuestion = {
      ...
      min_value: ["rating", "scale"].includes(newQuestion.question_type)
        ? newQuestion.min_value
        : undefined,
      max_value: ["rating", "scale"].includes(newQuestion.question_type)
        ? newQuestion.max_value
        : undefined,
  ...
  const handleUpdateQuestion = () => {
    if (editingQuestionIndex === null || !newQuestion.question_text.trim())
      return;
  ...
  const handleOpenDialog = async (survey?: SurveyData) => {
    if (survey) {
      ...
      try {
        ...
        setFormData({
          ...
          closes_at: surveyData.closes_at
            ? new Date(surveyData.closes_at)
            : null,
        ...
        {isEmployeeResponseMode && surveyToRespond ? (
                  ...
                  <Box key={question.id} sx={{ mb: 4 }}>
                    ...
                    {question.question_type === "text" && (
                      <TextField
                        fullWidth
                        placeholder={
                          question.placeholder_text || "Escriba su respuesta..."
                        }
                    ...
                    {question.question_type === "single_choice" &&
                      question.options && (
                        <RadioGroup
                          value={
                            employeeAnswers[question.id]?.answer_text || ""
                          }
                          onChange={(e) => {
                            setEmployeeAnswers((prev) => ({
                              ...prev,
                    ...
                    {question.question_type === "multiple_choice" &&
                      question.options && (
                        <FormGroup>
                          {JSON.parse(question.options).map(
                            (option: string, optionIndex: number) => {
                              const selectedOptions = employeeAnswers[
                                question.id
                              ]?.selected_options
                                ? JSON.parse(
          ...
          <Box>
                  ...
                  <Grid size={{ xs: 12, md: 2 }}>
                    <FormControl fullWidth>
                      ...
                      <Select
                        ...
                        onChange={(e) =>
                          handleFilterChange("survey_type", e.target.value)
                        }
                      >
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {loading ? (
                        <TableRow>
                          <TableCell
                            colSpan={user?.role === "employee" ? 5 : 7}
                            align="center"
                          >
                            Cargando encuestas...
                          </TableCell>
                          ...
                          <TableRow key={employeeSurvey.id}>
                            ...
                            <TableCell>
                              <Typography variant="body2">
                                {employeeSurvey.survey?.course?.title ||
                                  "Sin curso"}
                              </Typography>
                            </TableCell>
                            <TableCell>
                              <Typography variant="body2">
                                {employeeSurvey.completed_at
                                  ? formatDateTime(employeeSurvey.completed_at)
                                  : "-"}
                              </Typography>
                            </TableCell>
                            <TableCell>
                          ...
                          <TableRow key={survey.id}>
                            ...
                            <TableCell>
                              <Chip
                                ...
                                color={
                                  survey.is_anonymous ? "success" : "default"
                                }
                                size="small"
                              />
                            </TableCell>
                            <TableCell>
                              <Chip
                                label={
                                  survey.allow_multiple_responses ? "Sí" : "No"
                                }
                                color={
                ...
                <Grid container spacing={3} sx={{ mt: 1 }}>
                  ...
                  <Grid size={{ xs: 12, md: 6 }}>
                    <DatePicker
                      ...
                      onChange={(date) =>
                        setFormData({ ...formData, closes_at: date })
                      }
                      ...
                      control={
                        <Switch
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              required_for_completion: e.target.checked,
                            })
                          }
                        />
                      }
                          ...
                          >
                            ...
                            <Box display="flex" gap={1}>
                              ...
                              <IconButton
                                ...
                                disabled={
                                  index === 0 || editingQuestionIndex !== null
                                }
                              >
                                ↑
                              </IconButton>
                        ...
                        <Grid container spacing={2}>
                          ...
                          {["multiple_choice", "single_choice"].includes(
                            ...
                            <Grid size={12}>
                              ...
                              {JSON.parse(newQuestion.options || "[]").map(
                    ...
                    <Grid container spacing={3}>
                      ...
                      <Grid size={{ xs: 12, md: 6 }}>
                        ...
                        <Chip
                          label={
                            statusConfig[
                              viewingSurvey.status as keyof typeof statusConfig
                            ]?.label
                          }
                          color={
                            statusConfig[
            ...
            <Dialog
              ...
              onClose={() => setOpenResponsesDialog(false)}
              maxWidth="lg"
              fullWidth
            >
              <DialogTitle>
                ...
                {userSurveys && userSurveys.length > 0 ? (
                  <TableContainer component={Paper}>
                    <Table>
                      ...
                      <TableBody>
                        {userSurveys &&
                          userSurveys.map((employeeResponse, index) => (
                            <TableRow
                              key={`${employeeResponse.user_id}-${index}`}
                            >
                              <TableCell>
                              ...
                              <TableCell>
                                {employeeResponse.response_time_minutes
                                  ? `${employeeResponse.response_time_minutes} min`
                                  : "-"}
                              </TableCell>
                              <TableCell>
              ...
              <DialogActions>
                <Button onClick={() => setOpenResponsesDialog(false)}>
                  Cerrar
                </Button>
              </DialogActions>
            </Dialog>
            ...
            >
              ...
              <DialogContent>
                {selectedEmployeeResponse && (
                  <Box>
                        ...
                        <Box display="flex" flexDirection="column" gap={2}>
                          ...
                          <Box display="flex" flexWrap="wrap" gap={2}>
                            <Box flex="1" minWidth="200px">
                              ...
                              <Typography variant="body1">
                                {selectedEmployeeResponse.cargo ||
                                  "No especificado"}
                              </Typography>
                            </Box>
                            <Box flex="1" minWidth="200px">
                              ...
                              <Chip
                                label={
                                  selectedEmployeeResponse.submission_status
                                }
                                color={
                                  selectedEmployeeResponse.submission_status ===
                                  "completed"
                    ...
                    {selectedEmployeeResponse.answers &&
                    selectedEmployeeResponse.answers.length > 0 ? (
                      <Box>
                        {selectedEmployeeResponse.answers.map(
                          (answer, index) => (
              ...
              <DialogActions>
                <Button onClick={() => setOpenDetailedResponseDialog(false)}>
                  Cerrar
                </Button>
              </DialogActions>
            </Dialog>
            ...
            >
              <DialogTitle>
                ...
                >
                  ...
                  <Typography variant="body2" color="text.secondary">
                    {selectedUsers.length} de {getFilteredUsers().length}{" "}
              ...
              <DialogContent>
                <Typography variant="body2" color="text.secondary" gutterBottom>
                  Selecciona los usuarios a los que deseas asignar esta encuesta
                  general.
                </Typography>
                ...
                {selectedUsers.length > 0 && (
                  <Alert severity="info" sx={{ mt: 2 }}>
                    <Typography variant="body2">
                      ...
                      seleccionado(s) para asignar la encuesta
                    </Typography>
                  </Alert>
                )}
              </DialogContent>
            ...
            >
              ...
              <DialogContent>
                <Typography
                  ...
                  sx={{ mb: 2 }}
                >
                  ¿Está seguro de que desea eliminar la encuesta{" "}
                  <strong>"{deletingSurvey?.title}"</strong>?
                </Typography>
                <Alert severity="warning" sx={{ mb: 2 }}>
                  <Typography variant="body2">
                    <strong>Importante:</strong> Esta acción no se puede
                    deshacer.
                  </Typography>
                </Alert>
                <Alert severity="info">
                  <Typography variant="body2">
                    <strong>Nota:</strong> Si la encuesta tiene respuestas de
                    usuarios, no podrá ser eliminada. En ese caso, considere
                    cambiar su estado a "Archivada" en lugar de eliminarla.
                  </Typography>
                </Alert>
              </DialogContent>
              <DialogActions sx={{ px: 3, pb: 2 }}>

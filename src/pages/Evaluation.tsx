...
const EvaluationsManagement: React.FC = () => {
  ...
  const [evaluationToRespond, setEvaluationToRespond] =
    useState<Evaluation | null>(null);
  ...
  const [openDeleteQuestionDialog, setOpenDeleteQuestionDialog] =
    useState(false);
  const [deletingQuestionIndex, setDeletingQuestionIndex] =
    useState<number>(-1);
  ...
  const [resettingEvaluation, setResettingEvaluation] =
    useState<Evaluation | null>(null);
  ...
  const fetchEvaluations = useCallback(async () => {
    try {
      ...
      showSnackbar(
        "No se pudieron cargar las evaluations. Verifique su conexión e intente nuevamente.",
        "error",
      );
    } finally {
      setLoading(false);
  ...
  const handleSubmitEvaluation = useCallback(
    async (isAutoSubmit: boolean = false) => {
      ...
      if (!isAutoSubmit) {
        const requiredQuestions = (evaluationToRespond.questions || []).filter(
          (q) => q.required,
        );
        const missingAnswers = requiredQuestions.filter(
          (q) => !employeeAnswers[q.id],
        );
      ...
      try {
        ...
        const answers = Object.entries(employeeAnswers).map(
          ([questionId, answer]) => {
            const question = evaluationToRespond.questions?.find(
        ...
        const response = await api.post(
          `/evaluations/${evaluationToRespond.id}/submit`,
          answers,
        );
  ...
  const fetchEvaluationToRespond = useCallback(
    async (evaluationId: number) => {
      try {
        ...
        const userStatusResponse = await api.get(
        ...
        const isInProgress =
          employeeResponses[evaluationId]?.status === "in_progress" ||
        ...
        if (
          errorDetail.includes("Maximum attempts") &&
          ...
          showSnackbar(
            errorDetail ||
              "No se pudo cargar la evaluación. Verifique su conexión e intente nuevamente.",
            "error",
          );
        }
      } finally {
  ...
  const confirmResetEvaluation = async () => {
    ...
    try {
      ...
      await api.post(
        `/evaluations/${resettingEvaluation.id}/reset-status/${user.id}`,
      );
  ...
  const saveEvaluationProgress = async (showMessage: boolean = false) => {
    ...
    try {
      ...
      const user_answers = Object.entries(employeeAnswers).map(
        ([questionId, answer]) => {
          const question = evaluationToRespond.questions?.find(
            (q) => q.id === parseInt(questionId),
          );
    ...
    try {
      ...
      setFormData({
        ...
        expires_at: evaluationData.expires_at
          ? evaluationData.expires_at.split("T")[0]
          : "",
        questions:
          evaluationData.questions?.map((q: Question) => {
      ...
      showSnackbar(
        "No se pudieron cargar los detalles de la evaluación. Verifique su conexión e intente nuevamente.",
        "error",
      );
    }
  };
  ...
  const handleSaveEvaluation = async () => {
    try {
      ...
      const transformedQuestions = (formData.questions || []).map(
        (question) => {
          ...
          if (
            ...
            transformedQuestion.options = question.answers.map(
              (answer) => answer.answer_text,
            );
            const correctAnswer = question.answers.find(
              (answer) => answer.is_correct,
            );
            transformedQuestion.correct_answer = correctAnswer
      ...
      {!isEmployeeResponseMode && (
        <>
                    ...
                    .map((evaluation) => (
                      <TableRow key={evaluation.id}>
                        ...
                        <TableCell align="center">
                          {user?.role === "employee" &&
                          employeeResponses[evaluation.id]?.status ===
                            "in_progress" ? (
                            <IconButton
                              color="warning"
                              onClick={() =>
                                fetchEvaluationToRespond(evaluation.id)
                              }
                              size="small"
                              ...
                              {(user?.role === "admin" ||
                                ...
                                !employeeResponses[evaluation.id]?.passed && (
                                  <IconButton
                                    color="warning"
                                    onClick={() =>
                                      handleResetEvaluation(evaluation)
                                    }
                                    size="small"
              ...
              <Box sx={{ mt: 2 }}>
                ...
                <Box sx={{ mt: 4 }}>
                  ...
                  {formData.questions && formData.questions.length > 0 ? (
                        ...
                        <Paper key={index} sx={{ p: 2, mb: 2 }}>
                          ...
                          >
                            <Box sx={{ flex: 1 }}>
                              ...
                              >
                                Tipo: {question.question_type} | Puntos:{" "}
                                {question.points}
                    ...
                    <Alert severity="info">
                      No hay preguntas agregadas. Haz clic en "Agregar Pregunta"
                      para comenzar.
                    </Alert>
                  )}
                </Box>
              ...
              {viewingEvaluation && (
                <Box sx={{ mt: 2 }}>
                  ...
                  >
                    Estado:{" "}
                    <Chip
                      label={getStatusLabel(viewingEvaluation.status)}
                      color={getStatusColor(viewingEvaluation.status)}
                      size="small"
                    />
                  </Typography>
                  ...
                  <Typography variant="body1" sx={{ mt: 2 }}>
                    ...
                    {viewingEvaluation.passing_score}%
                  </Typography>
                  <Typography variant="body1" sx={{ mt: 1 }}>
                    <strong>Máximo intentos:</strong>{" "}
                    {viewingEvaluation.max_attempts}
                  </Typography>
                  <Typography variant="body1" sx={{ mt: 1 }}>
                    <strong>Creado:</strong>{" "}
                    {formatDateTime(viewingEvaluation.created_at)}
                  </Typography>
                  {viewingEvaluation.questions &&
                    viewingEvaluation.questions.length > 0 && (
                      <Box sx={{ mt: 3 }}>
                        <Typography variant="h6" gutterBottom>
          ...
          >
            ...
            <DialogContent>
              <Typography>
                ...
                {deletingEvaluation?.title}"? Esta acción no se puede deshacer.
              </Typography>
            </DialogContent>
            <DialogActions>
          ...
          >
            ...
            <DialogContent>
              <Typography id="reset-dialog-description">
                ¿Está seguro de que desea reiniciar esta evaluación? Esto
                cambiará el estado de la evaluación a "No iniciada" y eliminará
                todas las respuestas y puntuaciones previas.
              </Typography>
            </DialogContent>
            <DialogActions>
              ...
              <Button
                ...
                startIcon={
                  resetLoading ? <CircularProgress size={20} /> : <RestartAlt />
                }
              >
                                  ...
                                  const updatedAnswers = [
                                    {
                                      answer_text: "Verdadero",
                                      is_correct:
                                        e.target.value === "Verdadero",
                                      order_index: 0,
                                    },
                                    {
                                      answer_text: "Falso",
                                      is_correct: e.target.value === "Falso",
                                      order_index: 1,
                                    },
                                  ];
                                  setQuestionFormData({
            ...
            <DialogActions>
              ...
              <Button
                ...
                disabled={
                  ...
                  ((questionFormData.question_type === "multiple_choice" ||
                    questionFormData.question_type === "single_choice") &&
                    !questionFormData.answers.some((a) => a.is_correct))
                }
              >
          ...
          >
            ...
            <DialogContent>
              <Typography>
                ¿Está seguro de que desea eliminar esta pregunta? Esta acción no
                se puede deshacer.
              </Typography>
            </DialogContent>
            <DialogActions>
              <Button onClick={() => setOpenDeleteQuestionDialog(false)}>
                Cancelar
              </Button>
              <Button
          ...
          >
            ...
            <DialogContent>
              <Typography>
                ¿Está seguro de que desea eliminar esta respuesta? Esta acción
                no se puede deshacer.
              </Typography>
            </DialogContent>
            <DialogActions>
              <Button onClick={() => setOpenDeleteAnswerDialog(false)}>
                Cancelar
              </Button>
              <Button
          ...
          {timeExpired && (
            <Alert severity="error" sx={{ mb: 3 }}>
              <Typography variant="h6">
                ⏰ El tiempo ha expirado. La evaluación se ha guardado
                automáticamente.
              </Typography>
            </Alert>
          )}
                  ...
                  {(evaluationToRespond.questions || [])
                    ...
                    .map((question, index) => (
                      <Box key={question.id} sx={{ mb: 4 }}>
                        ...
                        {index <
                          (evaluationToRespond.questions || []).length - 1 && (
                          <Divider sx={{ mt: 3 }} />
                        )}
                      </Box>
                    ))}
        ...
        <DialogContent sx={{ pt: 2 }}>
          <Box sx={{ mb: 3 }}>
            <Typography variant="body1" sx={{ mb: 2, lineHeight: 1.6 }}>
              Has alcanzado el número máximo de intentos permitidos para esta
              evaluación.
            </Typography>

...
const WorkerDetail: React.FC = () => {
  ...
  const [reassignType, setReassignType] = useState<'course' | 'survey' | 'evaluation'>('course');
  ...
  const handleOpenReassignDialog = async (type: 'course' | 'survey' | 'evaluation') => {
  ...
  return (
    <Box p={3}>
        ...
        <CardContent>
          <Grid container spacing={3}>
            <Grid size={{ xs: 12, md: 6 }}>
              ...
              <Typography><strong>Nombre:</strong> {workerData.worker.full_name}</Typography>
              <Typography><strong>Documento:</strong> {workerData.worker.document_number}</Typography>
              <Typography><strong>Email:</strong> {workerData.worker.email}</Typography>
            </Grid>
            <Grid size={{ xs: 12, md: 6 }}>
              <Typography variant="h6" gutterBottom>
                Información Laboral
              </Typography>
              <Typography><strong>Cargo:</strong> {workerData.worker.position}</Typography>
              <Typography><strong>Departamento:</strong> {workerData.worker.department}</Typography>
              <Box mt={1}>
                <Chip
                ...
                {workerData.evaluations.length === 0 ? (
                  ...
                  workerData.evaluations.map((evaluation) => (
                    <TableRow key={evaluation.user_evaluation_id}>
                      ...
                      <TableCell>{formatDate(evaluation.completed_at)}</TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
      ...
      {/* Reassign Dialog */}
      <Dialog open={openReassignDialog} onClose={() => setOpenReassignDialog(false)} maxWidth="sm" fullWidth>
        <DialogTitle>
          Asignar {reassignType === 'course' ? 'Curso' : reassignType === 'survey' ? 'Encuesta' : 'Evaluación'}
        </DialogTitle>
        <DialogContent>
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField
                {...params}
                label={`Seleccionar ${reassignType === 'course' ? 'curso' : reassignType === 'survey' ? 'encuesta' : 'evaluación'}`}
                fullWidth
                margin="normal"
              />
            )}
